...
def enhancedFeatureExtractorDigit(datum):
    ...
    ***
    this function add a new feature descirbing the number of connected regions the digit type has
    ***
    ##
    """
    ...
    "*** YOUR CODE HERE ***"
    """
    ***USELESS CODE FOR BACKUP***
    connectRegion = []
    coords = features.sortedKeys()
    #print coords
    ...
    def getNeighbors(coord):
        ...
        if coord[0] > 0:
            tempCoords.append( (coord[0] - 1, coord[1]) )
        if coord[1] > 0:
            tempCoords.append( (coord[0], coord[1] - 1) )
        ...
        for tempCoord in tempCoords:
            if features[tempCoord] > 0:
                tempCoords.remove(tempCoord)

        return tempCoords

    
    #def findConnectRegion(coord):
     #   connectRegion.append(coord)
       # tempCoords = [(coord[0] - 1, coord[1]), (coord[0] + 1, coord[1]), (coord[0], coord[1] - 1), (coord[0], coord[1] + 1)]
        #for tempCoord in tempCoords:
         #   if tempCoord not in coords:
           #     tempCoords.remove(tempCoord)
        #for tempCoord in tempCoords:
          #  if features[tempCoord] == 0 and tempCoord not in connectRegion:
            #    findConnectRegion(tempCoord)
    ...
    for coord in coords:
        ...
        if features[coord] == 0:
            ...
            while 1:
                numOfLoop += 1
                #print numOfLoop
                tempLen = len(connectRegion)
                for tempCoord in connectRegion:
                    neighbors = getNeighbors(tempCoord)
                    #print neighbors
                    for neighbor in neighbors:
                        if neighbor not in connectRegion:
                            connectRegion.append(neighbor)
                if tempLen == len(connectRegion):
                    break
            break
    ...
    #print count
    #print len(connectRegion)
    #input()
    if count == len(connectRegion):
        features['connectRegion'] = 1
    else:
        features['connectRegion'] = 2
    ...
    def findConnectRegion(coord):
        ...
        for neighbor in neighbors:
            if datum.getPixel(neighbor[0], neighbor[1]) == 0 and neighbor not in visitedCoords:
                findConnectRegion(neighbor)
    
    numOfConnectRegion = 0

    for coord in coords:
        if datum.getPixel(coord[0], coord[1]) == 0 and coord not in visitedCoords:
            numOfConnectRegion += 1
            findConnectRegion(coord)

    if numOfConnectRegion > 1:
        features['numOfConnectRegion'] = numOfConnectRegion
    else:
        features['numOfConnectRegion'] = 0
    return features
