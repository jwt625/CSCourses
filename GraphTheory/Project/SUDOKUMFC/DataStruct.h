...
//iMAT class describes a 2-D 9x9 matrix of type int
//WITH METHODS:
//int rows()
//int cols()
...
public:
	...
	iMAT(int row, int col):m_row(row), m_col(col){}
	int & operator()(int i, int j){return m_mat[i][j];}
	bool operator==(iMAT tempM);
	int coeff(int i, int j)const{return m_mat[i][j];}
	int & coeffRef(int i, int j){return m_mat[i][j];}
